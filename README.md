...
**背景色问题（已确认，需要在 Rive 编辑器里修）**：`dog.riv` 的 `Hond`
artboard 内部画了一块紫色/淡蓝紫色的背景矩形，这是素材自带的，不是窗口没做
透明。查过 Rive 官方 Layout 文档确认：`fit` / `alignment` 这些运行时布局
参数只能控制"整个 artboard 在 view 里怎么缩放/裁切/对齐"，没有能单独隐藏
artboard 内某一层的运行时 API。想要透明背景，需要回到 Rive 编辑器里把这块
背景矩形删掉（或者 Fill 透明度调成 0），重新导出覆盖 `Resources/dog.riv`，
代码这边不需要改。

**"狗看起来太小"（已在代码里解决一半）**：默认 `fit` 是 `.contain`（官方
文档确认的默认值），会把整个 artboard——包括背景矩形四周的留白——完整塞进
170×150 画布，狗自然显得小。`BichonView.swift` 里已经改成 `fit: .cover`
（同一份文档确认的合法取值），按小边对齐、裁掉多余部分，相当于放大画面填满
画布，狗会明显变大；代价是背景矩形边缘会被裁掉一部分而不是完全消失——彻底
去掉背景还是要靠上面说的 Rive 编辑器改法。
